...
stages. The hook instrumentation is planned in the inductor layer of the compile stack.
This will enable the instrumentation to be easily adopted to other accelrators as well.
<!--
Currently, instrumentation hooks will target the
...
```
-->
 The aim of this profiler is to provide the user with a way to specify one or more operators they want to investigate, and obtain the following insights for those ops:
...
any compiler pass as specified by the user.
With the transition to KTIR (RFC 0682) we wish to extend the tracking capability to 
capture the KTIR translation as well. Extend debug handle embedding to KTIR operations once the KTIR
